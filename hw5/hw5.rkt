...
(define (mupllist->racketlist es)
  (cond [(aunit? es) null]
        [(apair? es) (cons (apair-e1 es) (mupllist->racketlist (apair-e2 es)))]
        [#t error "Expected a mupllist"]))
  ...
  (cond [(var? e) 
        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1) (int? v2))
               ...
               (error "one of the two first arguments of MUPL ifgreater is not an MUPL int")))]
         ...
         (let ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? v1)
               (let* ([env (closure-env v1)]
                      [func (closure-fun v1)]
                      [env1 (cons (cons (fun-formal func) v2) env)]
                      [env2 (if (equal? #f (fun-nameopt func))
                                env1
                                (cons (cons (fun-nameopt func) v1) env1))])
                   (eval-under-env (fun-body func) env2))
               (error "MUPL call expects closure as first argument")))]
        ...
        [(fst? e)
         (let ([v (eval-under-env (fst-e e) env)])
           (if (apair? v)
               (apair-e1 v)
               (error "MUPL fst expects apair")))]
        
        [(snd? e)
         (let ([v (eval-under-env (snd-e e) env)])
           (if (apair? v)
               (apair-e2 v)
               (error "MUPL snd expects apair")))]
...
(define mupl-map (fun "mupl-map" "f"
                       (fun "mapf" "mlist" 
                            (ifaunit (var "mlist")
                                    (aunit)
                                    (apair 
                                     (call (var "f") (fst (var "mlist")))
                                     (call (var "mapf") (snd (var "mlist"))))))))
...
(define (compute-free-vars e) 
  (letrec ([get-free-vars 
            (lambda (e)
              (cond 
                [(var? e) (set (var-string e))]
                [(int? e) (set)]
                [(add? e) (set-union (get-free-vars (add-e1 e)) (get-free-vars (add-e2 e)))]
                ...
                [(fun? e) (let* ([body-vars (get-free-vars (fun-body e))]
                                 [vars (set-remove body-vars (fun-formal e))])
                            (if (equal? #f (fun-nameopt e))
                                vars
                                (set-remove vars (fun-nameopt e))))]
                [(call? e) (set-union (get-free-vars (call-funexp e)) (get-free-vars (call-actual e)))]
                [(mlet? e) (set-union
                            (get-free-vars (mlet-e e))
                            (set-remove (get-free-vars (mlet-body e)) (mlet-var e)))]
                [(apair? e) (set-union (get-free-vars (apair-e1 e)) (get-free-vars (apair-e2 e)))]
...
(define (eval-under-env-c e env) 
  (cond [(var? e) (envlookup env (var-string e))]
        ...
        [(fun-challenge? e) (closure
                             (filter
                              (lambda (el) (set-member? (fun-challenge-freevars e) (car el)))
                              env)
                             (fun 
                              (fun-challenge-nameopt e)
           ...
           (if (closure? v1)
               (let* ([env (closure-env v1)]
                      ...
                      [env2 (if (equal? #f (fun-nameopt func))
                                env1
                                (cons (cons (fun-nameopt func) v1) env1))])
                   (eval-under-env-c (fun-body func) env2))
               (error "MUPL call expects closure as first argument")))]
